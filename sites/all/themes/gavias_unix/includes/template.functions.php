<?php
function gavias_unix_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'gavias_unix');
  return $base_url . '/' . $theme_path . '/';
}
global $gva_node_index;
function gavias_unix_preprocess_node(&$variables) {
  global $gva_node_index;
  ...
  $variables['theme_uri'] = base_path() . drupal_get_path('theme', 'gavias_unix');
}

function gavias_unix_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function gavias_unix_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function gavias_unix_preprocess_breadcrumb(&$variables){
...
function gavias_unix_preprocess_node__course(&$variables) {
  $view_mode = $variables['view_mode']; 
  $allowed_view_modes = ['full']; 
  if(in_array($view_mode, $allowed_view_modes)) {
    $allowed_regions = ['lessons'];
    gavias_unix_add_regions_to_node($allowed_regions, $variables);
  }
}
