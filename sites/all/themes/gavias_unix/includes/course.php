<?php
function gavias_unix_check_registered_course($uid, $course_id){
...
function gavias_unix_preprocess_course(&$vars){
  ...
  $uid = $current_user->id();
  if(gavias_unix_check_registered_course($uid, $id)){
...
function gavias_unix_preprocess_commerce_product__course(&$vars){
  $vars = gavias_unix_preprocess_course($vars);
}
function gavias_unix_preprocess_commerce_product__course__teaser(&$vars){
  $vars = gavias_unix_preprocess_course($vars);
}
function gavias_unix_preprocess_commerce_product__course__teaser_2(&$vars){
  $vars = gavias_unix_preprocess_course($vars);
}

function gavias_unix_preprocess_node__lesson(&$variables) {
  $variables['#attached']['library'][] = 'gavias_unix/gavias-lesson-video';
  ...
  if($node = $variables['node']){
    ...
    if(in_array("administrator", $role)){
      ...
      if($lesson_access == 'registered'){
        if( $user->id() && gavias_unix_check_registered_course($user->id(), $course_id) ){
          $lesson_icon = 'fa fa-check';
          if(isset($variables['content']['field_lesson_content'])){
            $lesson_content = $variables['content']['field_lesson_content'];
          }    
