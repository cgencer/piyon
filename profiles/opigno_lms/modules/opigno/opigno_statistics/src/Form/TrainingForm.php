...
class TrainingForm extends FormBase {
   ...
   *   Render array.
   */
  protected function buildTrainingsProgress(GroupInterface $group, $datetime) {
...
AND gc.gid = :gid",
    ...
    $table = [
      '#type' => 'table',
      '#attributes' => [
        'class' => ['statistics-table', 'training-content-list', 'table-striped'],
      ],
      '#header' => [
        $this->t('Step'),
        $this->t('% Completed'),
...
AND gc.gid = :gid",
  ...
  protected function buildUsersResultsClass(GroupInterface $group, $lp_id = NULL) {
    ...
    if (empty($members_ids)) $members_ids[] = 0;
