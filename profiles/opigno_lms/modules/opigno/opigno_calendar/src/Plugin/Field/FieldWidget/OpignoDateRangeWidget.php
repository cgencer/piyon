...
class OpignoDateRangeWidget extends WidgetBase {
   ...
   *
   * @param array $wrapper_name
   *   Datetime field wrapper.
   *
   * @return DrupalDateTime
   *   Datetime object.
   */
  public static function createDateTimeFromWrapper($wrapper) {
    ...
    foreach ($values as &$item) {
      ...
      if (!empty($item['end_value_wrapper'])) {
        ...
        if (isset($date) && $end_date < $date) {
          $form_state->setError($form, t('The end date cannot be before the start date'));
        }
      }
    }
    return $values;
