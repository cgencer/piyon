...
//      // registration UUID.
//      return self::saveRegistrationFromSession($activity);
    }
  ...
  protected function getLrsConnection() {
    ...
    if (!class_exists('TinCan\\Version')) {
      $messenger->addError(t(
        ...
        ['@tincanphp_library' => Link::fromTextAndUrl(t('TinCanPHP library'), Url::fromUri('https://github.com/RusticiSoftware/TinCanPHP'))]
      ));
      return FALSE;
    }
    ...
    if (empty($endpoint) || empty($username) || empty($password)) {
      $messenger->addWarning(t('Please configure first the Opigno TinCan API module. Go to @url',
        [
