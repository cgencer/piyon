...
use Drupal\flow_player_field\Plugin\Field\FieldFormatter\Video;
...
class FlowPlayerWysiwyg extends FilterBase implements ContainerFactoryPluginInterface
{
    ...
    public function process($text, $langcode)
    {
        ...
        foreach ($this->getValidMatches($text) as $source_text => $embed_data) {
            ...
            $embed_code = [
                '#type' => 'container',
                '#attributes' => [
                    'class' => ['flow-player-embed-container', Html::cleanCssIdentifier(sprintf('flow-player-field-provider-%s', $embed_data['video_id']))],
                ],
                'children' => $embed_code,
            ];
    ...
    public function getEmbedHtml($embed_data)
    {
