...
class WebformTCKNElement extends TextBase {
  ...
  public static function validateWebformTCKNElement(array &$element, FormStateInterface $form_state, array &$completed_form) {
    $invalid = ['11111111110','22222222220','33333333330','44444444440','55555555550','66666666660','7777777770','88888888880','99999999990','12345678910'];
    $name = $element['#name'];
    ...
    if (substr($entered, 0, 1) == 0) {
      $form_state->setErrorByName($name, t('TCKN can\'t start with 0.'));
    }
    if (strlen($entered) < 11) {
      $form_state->setErrorByName($name, t('TCKN can\'t be less than 11 characters.'));
    }
    if ($entered != $final) {
      $form_state->setErrorByName($name, t('TCKN doesn\'t validate.'));
    }
